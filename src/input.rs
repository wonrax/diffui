//! The app's single-line text input: iced's `text_input` widget, vendored to
//! give it the **stateful horizontal scroll** upstream lacks (its own
//! `// TODO: Add stateful horizontal scrolling offset`).
//!
//! Stock iced re-derives the scroll offset from the caret on every frame —
//! `offset = caret_x + headroom - width`, clamped to 0 — so clicking a
//! visible character in an overflowing value yanks that spot to the right
//! edge, and clicking anywhere in the first view-width of text snaps back to
//! the start. Editing a long revset meant the text jumping out from under
//! every caret click. Here the offset lives in `State` and only ever moves
//! the *minimum* needed to keep the caret in view (see [`reconcile_scroll`]);
//! hit-testing reads the stored offset so clicks resolve against the frame as
//! drawn. Everything else — styling types, focus operations, key handling —
//! matches upstream, and `Style`/`Catalog` are re-exported from the stock
//! widget so themes serve both.
//!
//! Vendored from iced rev `4255f61` (`widget/src/text_input.rs` + its
//! `cursor`/`editor`/`value` submodules, which expose only `pub(crate)`
//! mutators). When bumping iced, diff those files against this copy.
mod editor;
mod value;
...
// The theming surface is shared with the stock widget, so existing
// `text_input::Style`-returning closures (theme.rs, per-site overrides)
// work on both without conversion.
pub use iced::widget::text_input::{Catalog, Icon, Side, Status, Style, StyleFn};
...
use iced::advanced::{InputMethod, Layout, Shell, Widget, clipboard};
use iced::alignment;
use iced::keyboard;
...
/// Creates a new [`TextInput`] — a drop-in for `iced::widget::text_input`
/// (the stock helper this shadows).
    ...
    Renderer: text::Renderer,
{
    ...
    fn input_method<'b>(
        ...
        let (cursor_x, _) = measure_cursor_and_scroll_offset(text, text_bounds, caret_index);
        let scroll_offset = offset(state);
    ...
    ///
    /// [`Renderer`]: text::Renderer
    #[allow(clippy::too_many_arguments)] // upstream signature, kept verbatim
    pub fn draw(
        &self,
        tree: &Tree,
            ...
            .filter(|focus| focus.is_window_focused)
        {
            match state.cursor.state(value) {
                cursor::State::Index(position) => {
                    let offset = reconcile_scroll(state, text_bounds, position);
                ...
                cursor::State::Selection { start, end } => {
                    // The scroll follows `end` — the selection's moving side.
                    let offset = reconcile_scroll(state, text_bounds, end);
            ...
            // Blurred inputs rest at the value's start, like the stock
            // widget; the stored offset resets so refocusing starts stable.
...
pub struct State<P: text::Paragraph> {
    ...
    /// Horizontal scroll of the text within the bounds, in px — upstream's
    /// "TODO: Add stateful horizontal scrolling offset". `Cell` because the
    /// offset reconciles against the caret in `draw` (the one place every
    /// caret mutation — events, operations, pastes — funnels into next),
    /// where the state is only borrowed.
...
/// Headroom kept to the right of the caret so its stroke never sits clipped
/// against the bounds edge (upstream's `+ 5.0`).
const CURSOR_HEADROOM: f32 = 5.0;

/// The horizontal scroll currently on screen. Hit-testing goes through this —
/// a click must resolve against the frame as drawn, never a recomputed one.
fn offset<P: text::Paragraph>(state: &State<P>) -> f32 {
    if state.is_focused() {
...
/// Reconcile the stored scroll with the caret at `focus_index`: clamp to the
/// text's extent (edits shrink it), then scroll the *minimum* needed to keep
/// the caret in view. Moving the caret within the visible text therefore
/// leaves the view still — unlike the stock widget, which re-derives the
/// offset from the caret alone and yanks any clicked position to the right
/// edge (and everything else back to the start).
fn reconcile_scroll<P: text::Paragraph>(
    state: &State<P>,
    text_bounds: Rectangle,
    focus_index: usize,
) -> f32 {
    let (caret_x, _) =
        measure_cursor_and_scroll_offset(state.value.raw(), text_bounds, focus_index);
    let max_offset = (state.value.raw().min_width() + CURSOR_HEADROOM - text_bounds.width).max(0.0);
    let mut offset = state.scroll_offset.get().clamp(0.0, max_offset);
    if caret_x < offset {
        offset = caret_x;
    } else if caret_x + CURSOR_HEADROOM > offset + text_bounds.width {
        offset = (caret_x + CURSOR_HEADROOM - text_bounds.width).min(max_offset);
    }
